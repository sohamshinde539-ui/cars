...
const resolveMigrationImage = (image2, migration, withThumbnails) => {
  ...
  if (asset) {
    ...
    const edit = maybeInitialField && "edit" in maybeInitialField ? maybeInitialField?.edit : { x: 0, y: 0, zoom: 1, background: "transparent" };
...
const resolveMigrationLinkToMedia = (linkToMedia2, migration) => {
  const asset = migration._assets.get(linkToMedia2.id.config.id)?.asset;
